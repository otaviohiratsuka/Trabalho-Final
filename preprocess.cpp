...
struct AvaliacaoHash {
    size_t operator()(const Avaliacao& a) const {
        return hash<int>()(a.userId) ^ hash<int>()(a.movieId << 1);
    }
};
...
void preProcessar(const string& nomeArquivo, int maxLinhas) {
    ...
    auto dadosFiltrados = agruparAvaliacoesPorUsuario(avaliacoes, usuariosValidos, filmesValidos);

    escreverArquivoDeSaida(dadosFiltrados);

    cout << "Arquivo datasets/input.dat gerado com sucesso com limite de " << maxLinhas << " linhas lidas!\n";
}

int main() {
    preProcessar("ratings.csv", 100000);
    return 0;
}
